...
then
    ...
    # The RabbtMQ configuration
    sudo rabbitmqctl add_user aiidauser aiidapass123
    sudo rabbitmqctl add_vhost aiidavhost
    sudo rabbitmqctl set_user_tags aiidauser aiidatag
    sudo rabbitmqctl set_permissions -p aiidavhost aiidauser ".*" ".*" ".*"
fi
