            ...
            for model_name in model_order:
                ...
                for import_entry_id, entry_data in new_entries[model_name].items():
                    ...
                    if Model is models.DbGroup:
                        ...
                        # In case the export file was generate with the old Django schema
                        # and the metadata & transport_params are stored as a strings
                        if (isinstance(import_data['metadata'], six.string_types) or
                                isinstance(import_data['metadata'], six.binary_type)):
                            import_data['metadata'] = json.loads(import_data['metadata'])  # loads() can handle str and unicode/bytes

                        if (isinstance(import_data['transport_params'], six.string_types) or
                                isinstance(import_data['transport_params'], six.binary_type)):
                            import_data['transport_params'] = json.loads(import_data['transport_params'])
                ...
                if model_name == NODE_ENTITY_NAME:
                    ...
                    for o in objects_to_create:
                        ...
                        if extras_mode_new == 'import':
                            ...
                            # from here
                            deserialized_extras = {key: value for key, value in deserialized_extras.items() if
                                                   not
                                                   key.startswith('_aiida_')}
                            if o.node_type.endswith('code.Code.'):
                                deserialized_extras = {key: value for key, value in deserialized_extras.items()
                                                       if not
                                                       key == 'hidden'}
                            # till here
                            o.extras = dict()
