...
class TestSimple(AiidaTestCase):
    ...
    def test_2(self):
        ...
        try:
            ...
            with open(os.path.join(unpack_tmp_folder, 'metadata.json'), 'r') as f:
                metadata = json.load(f)
            metadata['export_version'] = 0.0
            with open(os.path.join(unpack_tmp_folder, 'metadata.json'), 'w') as f:
    ...
    def test_3(self):
        ...
        try:
            ...
            with open(unpack.get_abs_path('data.json'), 'r') as f:
                metadata = json.load(f)
            metadata['links_uuid'].append({'output': sd.uuid, 'input': 'non-existing-uuid', 'label': 'parent'})
    ...
    def test_4(self):
        ...
        export_tree([sd], folder=folder, silent=True, allowed_licenses=['GPL'])
        # Folder should contain two files of metadata + nodes/
        self.assertEquals(len(folder.get_content_list()), 3)

        folder = SandboxFolder()
        export_tree([sd], folder=folder, silent=True, forbidden_licenses=['Academic'])
        # Folder should contain two files of metadata + nodes/
        self.assertEquals(len(folder.get_content_list()), 3)

        folder = SandboxFolder()
        with self.assertRaises(LicensingException):
            export_tree([sd], folder=folder, silent=True, allowed_licenses=['CC0'])

        folder = SandboxFolder()
        with self.assertRaises(LicensingException):
            export_tree([sd], folder=folder, silent=True, forbidden_licenses=['GPL'])
        ...
        folder = SandboxFolder()
        with self.assertRaises(LicensingException):
            export_tree([sd], folder=folder, silent=True, allowed_licenses=cc_filter)

        folder = SandboxFolder()
        with self.assertRaises(LicensingException):
            export_tree([sd], folder=folder, silent=True, forbidden_licenses=gpl_filter)

        folder = SandboxFolder()
        with self.assertRaises(LicensingException):
            export_tree([sd], folder=folder, silent=True, allowed_licenses=crashing_filter)

        folder = SandboxFolder()
        with self.assertRaises(LicensingException):
            export_tree([sd], folder=folder, silent=True, forbidden_licenses=crashing_filter)
    ...
    def test_5(self):
        ...
        try:
            ...
            for uuid in uuids_u2:
                self.assertEquals(load_node(uuid).get_user().email, get_configured_user_email())
        finally:
            # Deleting the created temporary folder
            shutil.rmtree(temp_folder, ignore_errors=True)
    ...
    def test_7(self):
        ...
        try:
            ...
            # At this point we export the generated data
            filename1 = os.path.join(temp_folder, "export1.tar.gz")
            export([sd1, jc1, g1], outfile=filename1, silent=True)
    ...
    def test_reexport(self):
        ...
        try:
            ...
            # give some datetime:
            trial_dict.update({
                str(k): datetime(year=2017, month=np.random.randint(1, 12), day=np.random.randint(1, 28))
...
class TestComputer(AiidaTestCase):
    ...
    def test_same_computer_import(self):
        ...
        try:
            ...
            calc2 = JobCalculation()
            calc2.set_computer(self.computer)
            calc2.set_resources({"num_machines": 2, "num_mpiprocs_per_machine": 2})
            ...
            self.assertEqual(qb.count(), 0, "There should not be any computers" "in the database at this point.")

            # Check that there are no calculations
            qb = QueryBuilder()
            qb.append(JobCalculation, project=['*'])
            self.assertEqual(qb.count(), 0, "There should not be any " "calculations in the database at " "this point.")
            ...
            self.assertEqual(qb.count(), 1, "Only one calculation should be " "found.")
            self.assertEqual(six.text_type(qb.first()[0]), calc1_label, "The calculation label is not correct.")

            # Check that the referenced computer is imported correctly.
            qb = QueryBuilder()
            qb.append(Computer, project=['name', 'uuid', 'id'])
            self.assertEqual(qb.count(), 1, "Only one computer should be " "found.")
            self.assertEqual(six.text_type(qb.first()[0]), comp_name, "The computer name is not correct.")
            self.assertEqual(six.text_type(qb.first()[1]), comp_uuid, "The computer uuid is not correct.")
            ...
            self.assertEqual(six.text_type(qb.first()[0]), comp_name, "The computer name is not correct.")
            self.assertEqual(six.text_type(qb.first()[1]), comp_uuid, "The computer uuid is not correct.")
            self.assertEqual(qb.first()[2], comp_id, "The computer id is not correct.")
            ...
            self.assertEqual(qb.count(), 2, "Two calculations should be " "found.")
    ...
    def test_same_computer_different_name_import(self):
        ...
        try:
            ...
            qb.append(Computer, project=['name', 'uuid', 'id'])
            self.assertEqual(qb.count(), 1, "Only one computer should be " "found.")
            self.assertEqual(six.text_type(qb.first()[0]), comp1_name, "The computer name is not correct.")
    ...
    def test_different_computer_same_name_import(self):
        ...
        try:
            ...
            calc3.set_resources({"num_machines": 2, "num_mpiprocs_per_machine": 2})
            ...
            self.assertIn([calc2_label, comp1_name + COMP_DUPL_SUFFIX.format(0)], res,
                          "Calc-Computer combination not found.")
            self.assertIn([calc3_label, comp1_name + COMP_DUPL_SUFFIX.format(1)], res,
                          "Calc-Computer combination not found.")
        finally:
            # Deleting the created temporary folders
            shutil.rmtree(export_file_tmp_folder, ignore_errors=True)
    ...
    def test_correct_import_of_computer_json_params(self):
        ...
        try:
            ...
            comp1_transport_params = {u'key1': u'value1', u'key2': 2}
            self.computer.set_name(comp1_name)
            ...
            self.assertEqual(res['comp']['transport_params'], comp1_transport_params,
                             ...
                             "were found")
            self.assertEqual(res['comp']['_metadata'], comp1_metadata, "Not the expected metadata were found")
        finally:
            # Deleting the created temporary folders
            shutil.rmtree(export_file_tmp_folder, ignore_errors=True)
            shutil.rmtree(unpack_tmp_folder, ignore_errors=True)
    ...
    def test_import_of_django_sqla_export_file(self):
        ...
        for filename in ('export_dj_comp_test.aiida', 'export_sqla_comp_test.aiida'):
...
class TestLinks(AiidaTestCase):
    ...
    def test_complex_workflow_graph_links(self):
        ...
        try:
            ...
            qb.append(
                ...
                edge_filters={
                    'type': {
                        'in': (LinkType.INPUT.value, LinkType.CREATE.value, LinkType.RETURN.value, LinkType.CALL.value)
                    }
                })
            export_links = qb.all()
            ...
            for sub_scenarion_no in range(0, scenarios_number):
                ...
                try:
                    ...
                    export([export_node],
                           outfile=export_file,
                           silent=True,
                           **cg.get_scenarios_export_flags(scenario_name))
    ...
    def test_recursive_export_input_and_create_links_proper(self):
        ...
        try:
            ...
            wc1.add_link_from(ni1, 'ni1-to-wc1', link_type=LinkType.INPUT)
            c1.add_link_from(ni1, 'ni1-to-c1', link_type=LinkType.INPUT)

            # Connect the second data node to wc1 & c1
            wc1.add_link_from(ni2, 'ni2-to-wc1', link_type=LinkType.INPUT)
            c1.add_link_from(ni2, 'ni2-to-c1', link_type=LinkType.INPUT)

            # Connecting the first output node to wc1 & c1
            no1.add_link_from(wc1, 'output', link_type=LinkType.RETURN)
            no1.add_link_from(c1, 'output', link_type=LinkType.CREATE)

            # Connecting the second output node to wc1 & c1
            no2.add_link_from(wc1, 'output', link_type=LinkType.RETURN)
            no2.add_link_from(c1, 'output', link_type=LinkType.CREATE)
    ...
    def test_links_for_workflows(self):
        ...
        try:
            ...
            links_wanted = [
                l for l in self.get_all_node_links()
                if l[3] in (LinkType.CREATE.value, LinkType.INPUT.value, LinkType.RETURN.value)
            ]
    ...
    def test_double_return_links_for_workflows(self):
        ...
        try:
            ...
            links_wanted = [
                l for l in self.get_all_node_links() if l[3] in ('createlink', 'inputlink', 'returnlink', 'calllink')
            ]
            ...
            uuids_in_db = [str(uuid) for [uuid] in QueryBuilder().append(Node, project='uuid').all()]
    ...
    def test_that_input_code_is_exported_correctly(self):
        ...
        try:
            ...
            jc.set_resources({"num_machines": 1, "num_mpiprocs_per_machine": 1})
            jc.store()
            ...
            self.assertEquals(
                qb.count(), 1, "Expected to find one and only one link from "
                "code to the calculation node. {} found.".format(qb.count()))
        finally:
            shutil.rmtree(tmp_folder, ignore_errors=True)
...
class TestRealWorkChainExport(AiidaTestCase):
    ...
    def check_correct_export(self,
                             uuids_of_target_export_nodes,
                             uuids_of_target_import_nodes,
                             starting_node_to_draw_graph=None,
                             **export_args):
        import os, shutil, tempfile
        from aiida.orm.importexport import export
        from aiida.orm.querybuilder import QueryBuilder
        ...
        try:
            ...
            self.assertTrue(
                uuids_of_target_import_nodes.issubset(uuids_in_db), "The expected nodes ({}) where not "
                "found at the imported graph ({}).".format(uuids_of_target_import_nodes, uuids_in_db))
        finally:
            shutil.rmtree(tmp_folder, ignore_errors=True)
    ...
    class SubWorkChain(WorkChain):
        ...
        def start(self):
            ...
            if self.inputs.wf_counter > 0:
                inputs = {'wf_counter': Int(self.inputs.wf_counter - 1), 'a': self.inputs.a, 'b': self.inputs.b}
    ...
    def test_any_wc(self):
        ...
        _, _ = run(TestRealWorkChainExport.SubWorkChain, a=Int(1), b=Int(2), wf_counter=Int(2), _return_pid=True)
        ...
        for wf_uuid in tagret_wc_uuids:
            self.check_correct_export([wf_uuid],
                                      tagret_wc_uuids,
                                      call_reversed=True,
    ...
    def test_top_wc(self):
        ...
        ret_node, top_wf_pk = run(
            TestRealWorkChainExport.SubWorkChain, a=Int(1), b=Int(2), wf_counter=Int(2), _return_pid=True)
        # All the WorkCalculation UUIDs
        tagret_wc_uuids = set([str(uuid) for [uuid] in QueryBuilder().append(WorkCalculation, project=['uuid']).all()])
    ...
    def test_lower_return_node(self):
        ...
        ret_node, _ = run(TestRealWorkChainExport.SubWorkChain, a=Int(1), b=Int(2), wf_counter=Int(2), _return_pid=True)

        # All the WorkCalculation UUIDs
        tagret_wc_uuids = set([str(uuid) for [uuid] in QueryBuilder().append(WorkCalculation, project=['uuid']).all()])
        # Check that if we export the lower return node, all the graph is exported
        self.check_correct_export([ret_node['result'].uuid],
                                  tagret_wc_uuids,
                                  call_reversed=True,
                                  return_reversed=True,
                                  create_reversed=True,
