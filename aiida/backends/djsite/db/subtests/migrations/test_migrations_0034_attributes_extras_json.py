...
class TestTextFieldToJSONFieldMigration(TestMigrations):
    ...
    def setUpBeforeMigration(self):
        ...
        TestTextFieldToJSONFieldMigration.DbNode = self.apps.get_model('db', 'DbNode')
        TestTextFieldToJSONFieldMigration.DbAttributeBase = self.apps.get_model('db', 'DbAttribute')
        TestTextFieldToJSONFieldMigration.DbExtraBase = self.apps.get_model('db', 'DbExtra')
        TestTextFieldToJSONFieldMigration.DbUser = self.apps.get_model('db', 'DbUser')
        TestTextFieldToJSONFieldMigration.DbComputer = self.apps.get_model('db', 'DbComputer')
        ...
        attrdatatype_choice = (('float', 'float'), ('int', 'int'), ('txt', 'txt'), ('bool', 'bool'), ('date', 'date'),
            ...
            if mainitem['datatype'] == 'none':
                ...
                if expected_set != received_set:
                    ...
                    msg = ("Wrong list elements stored in {} for "
                           "{}key='{}' ({} vs {})".format(sourcestr, subspecifier_string, mainitem['key'], expected_set,
                                                          received_set))
                    if lesserrors:
                ...
                for firstsubk, firstsubv in firstlevelsubdict.items():
                    ...
                    newsubitems = {
                        k[len(firstsubk) + len(sep):]: v for k, v in subitems.items() if k.startswith(firstsubk + sep)
                    }
                ...
                if len(firstlevelsubdict) != mainitem['ival']:
                    ...
                    msg = ("Wrong dict length stored in {} for "
                           "{}key='{}' ({} vs {})".format(sourcestr, subspecifier_string, mainitem['key'],
                                                          len(firstlevelsubdict), mainitem['ival']))
                    if lesserrors:
                        AIIDA_LOGGER.error(msg)
                    else:
                        raise DeserializationException(msg)
        ...
        def deserialize_attributes(data, sep, original_class=None, original_pk=None):
            ...
            for k, v in found_mainitems.items():
                ...
                retval[k] = _deserialize_attribute(
                    mainitem=v,
                    subitems=found_subitems[k],
                    sep=sep,
                    original_class=original_class,
                    original_pk=original_pk)
        ...
        class DbMultipleValueAttributeBaseClass():
            # class DbMultipleValueAttributeBaseClass(TestTextFieldToJSONFieldMigration.DbAttributeBase):
            """
            ...
            def subspecifiers_dict(self):
                ...
                if self._subspecifier_field_name is None:
                    ...
                    return {self._subspecifier_field_name: getattr(self, self._subspecifier_field_name)}
            ...
            def create_value(cls, key, value, subspecifier_value=None, other_attribs={}):
                ...
                if cls._subspecifier_field_name is None:
                    ...
                    if issubclass(cls, DbAttribute):
                        new_entry = TestTextFieldToJSONFieldMigration.DbAttributeBase(key=key, **other_attribs)
                    else:
                        new_entry = TestTextFieldToJSONFieldMigration.DbExtraBase(key=key, **other_attribs)
                else:
                    ...
                    if issubclass(cls, DbAttribute):
                        new_entry = TestTextFieldToJSONFieldMigration.DbAttributeBase(key=key, **further_params)
                    else:
                        new_entry = TestTextFieldToJSONFieldMigration.DbExtraBase(key=key, **further_params)
            ...
            def get_query_dict(cls, value):
                ...
                if value is None:
                    ...
                    raise ValueError("Lists are not supported for getting the " "query_dict")
                elif isinstance(value, dict):
                    raise ValueError("Dicts are not supported for getting the " "query_dict")
                else:
                ...
                try:
                    if self.datatype == 'list' or self.datatype == 'dict':
                        ...
                        dballsubvalues = self.__class__.objects.filter(
                            key__startswith=prefix, **self.subspecifiers_dict).values_list(
                                'key', 'datatype', 'tval', 'fval', 'ival', 'bval', 'dval')
                        ...
                        return deserialize_attributes(
                            data, sep=self._sep, original_class=self.__class__,
            ...
            def del_value(cls, key, only_children=False, subspecifier_value=None):
                ...
                if cls._subspecifier_field_name is None:
                    ...
                    if subspecifier_value is None:
                        raise ValueError("You also have to specify a subspecifier value "
                                         "for class {} (the {})".format(cls.__name__, cls._subspecifier_field_name))
                    subspecifiers_dict = {cls._subspecifier_field_name: subspecifier_value}

                query = Q(key__startswith="{parentkey}{sep}".format(parentkey=key, sep=cls._sep), **subspecifiers_dict)
        ...
        class DbAttributeBaseClass(DbMultipleValueAttributeBaseClass):
            ...
            def get_all_values_for_nodepk(cls, dbnodepk):
                ...
                try:
                    return deserialize_attributes(data, sep=cls._sep, original_class=cls, original_pk=dbnodepk)
                except DeserializationException as exc:
                    exc = DbContentError(exc)
                    exc.original_exception = exc
                    raise exc
            ...
            @classmethod
            def reset_values_for_node(cls, dbnode, attributes, with_transaction=True, return_not_store=False):
                from django.db import transaction
                ...
                try:
                    ...
                    for k, v in attributes.items():
                        nodes_to_store.extend(cls.create_value(
                            k,
                            v,
        ...
        user = self.DbUser(email='aiida_migration@localhost', is_staff=False, is_active=True)
        user.save()

        computer = self.DbComputer(
        ...
        dbattr = self.DbAttributeBase()
        dbattr2 = DbAttribute()
        print("==========>", dbattr)

        for key in extras_to_set.keys():
            DbAttribute.set_value_for_node(node, key, extras_to_set[key])
        node.nodeversion = node.nodeversion + 1

        for key in extras_to_set.keys():
            DbExtra.set_value_for_node(node, key, extras_to_set[key])
        node.nodeversion = node.nodeversion + 1
